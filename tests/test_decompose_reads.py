import re
import pytest
from vmwhere.genotyper import (
    decompose_seq_with_motif_anchors,
    recompose_string_from_structure
)


@pytest.mark.parametrize("seq, motif, expected_substructure", [
    ("GGAA" * 4, "GGAA", ["4GGAA"]),
...
def test_decompose_and_recompose(seq, motif, expected_substructure):
    ...
    # When no motif is present, the function still returns a decomposed structure (not None)
    if expected_substructure is None:
        assert decomp_structure is not None
        recomposed = recompose_string_from_structure(decomp_structure)
        assert recomposed == subseq
        return
    else:
        # Check expected structure components
        for component in expected_substructure:
            assert component in decomp_structure
